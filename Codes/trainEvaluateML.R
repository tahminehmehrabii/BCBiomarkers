...
data21422 <- get_exprs_matrix("GSE21422")
expr21422 <- data21422$expr
gse21422 <- data21422$gse


write.table(expr21422, file = "GSE21422.txt", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

feat21422 <- fData(gse21422)
write.table(feat21422, file = "feat21422.txt", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

phen21422 <- pData(gse21422)
write.table(phen21422, file = "phen21422.txt", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

expr21422 <- cbind(ID=rownames(expr21422), expr21422)
write.table(expr21422, file = "expr21422.txt", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

feat21422 <- cbind(ID = feat21422[, "ID"], symbol = feat21422[, "Gene Symbol"])

expr21422 <- merge(feat21422, expr21422, by="ID")

rownames(expr21422) <- expr21422$ID

expr21422 <- expr21422[,-1]
...
expr21422 <- expr21422[which(expr21422$symbol %in% biomarkers),]

rownames(expr21422) <- NULL

expr21422 <- expr21422[!duplicated(expr21422$symbol),]


rownames(expr21422) <- expr21422$symbol

expr21422 <- expr21422[,-1]

expr21422 <- as.data.frame(t(expr21422))

expr21422 <- expr21422[,biomarkers]


for (j in 1:ncol(expr21422)) expr21422[,j] <- as.numeric(expr21422[,j])

expr21422 <- na.omit(expr21422)

expr21422 <- cbind(sample=rownames(expr21422), expr21422)

phen21422 <- cbind(sample=rownames(phen21422), phen21422)

phen21422 <- data.frame(
  sample = phen21422[, "geo_accession"], 
  group = phen21422[, "characteristics_ch1"]  
)

phen21422$group <- gsub("tissue: tumor", "Tumor", phen21422$group)
phen21422$group <- gsub("tissue: breast", "Normal", phen21422$group)

expr21422 <- merge(phen21422, expr21422, by="sample")

rownames(expr21422) <- expr21422$sample

expr21422 <- expr21422[,-1]

write.csv(expr21422, "trainEvaluateML_GSE21422.csv")
#############################################################################################
##############################################
trainSet <- as.data.frame(fread("corrected_training_set.csv"))
validSet <- as.data.frame(fread("corrected_validation_set.csv"))
gse21422 <- as.data.frame(fread("trainEvaluateML_GSE21422.csv"))

trainGroup <- trainSet$group
validGroup <- validSet$group
gse21422Group <- gse21422$group

rownames(trainSet) <- trainSet$sample
rownames(validSet) <- validSet$sample
rownames(gse21422) <- gse21422$V1

trainData <- trainSet[,-c(1:4)]
validData <- validSet[,-c(1:4)]
gse21422Data <- gse21422[,-c(1,2)]
...
gse21422Data <- gse21422Data[,which(colnames(gse21422Data) %in% biomarkers)]

trainData <- trainData[,biomarkers]
validData <- validData[,biomarkers]
gse21422Data <- gse21422Data[,biomarkers]

trainGroup <- as.factor(trainGroup)
validGroup <- as.factor(validGroup)
gse21422Group <- as.factor(gse21422Group)

levels(trainGroup) <- c("Normal", "Tumor")
levels(validGroup) <- c("Normal", "Tumor")
levels(gse21422Group) <- c("Normal", "Tumor")

train <- cbind(group=trainGroup, trainData)
valid <- cbind(group=validGroup, validData)
gse21422 <- cbind(group=gse21422Group, gse21422Data)

test <- rbind(gse21422)
accession <- "GSE21422"

write.csv(train, "train_data_GSE21422.csv")
write.csv(valid, "valid_data_GSE21422.csv")
write.csv(test, "test_data_GSE21422.csv")
...
write.csv(rf.train.cm$table, "rf.train.confusion.matrix_GSE21422.csv")
...
write.csv(rf.train.matrics, "rf.train.matrics_GSE21422.csv")
...
write.csv(rf.valid.cm$table, "rf.valid.confusion.matrix_GSE21422.csv")
...
write.csv(rf.valid.matrics, "rf.valid.matrics_GSE21422.csv")
...
write.csv(rf.test.cm$table, paste0("rf.test.confusion.matrix_GSE21422.csv"))
...
write.csv(rf.test.matrics, paste0("rf.test.matrics_GSE21422.csv"))
#######################
library(pROC)
library(caret)
library(MLmetrics)
...
write.csv(svm.train.cm$table, "svm.train.confusion.matrix_GSE21422.csv")
...
write.csv(svm.train.matrics, "svm.train.matrics_GSE21422.csv")
...
write.csv(svm.valid.cm$table, "svm.valid.confusion.matrix_GSE21422.csv")
...
write.csv(svm.valid.matrics, "svm.valid.matrics_GSE21422.csv")
...
write.csv(svm.test.cm$table, "svm.test.confusion.matrix_GSE21422.csv")
...
write.csv(svm.test.matrics, "svm.test.matrics_GSE21422.csv")
...
write.csv(nn.train.cm$table, "nn.train.confusion.matrix_GSE21422.csv")
...
write.csv(nn.train.matrics, "nn.train.matrics_GSE21422.csv")
...
write.csv(nn.valid.cm$table, "nn.valid.confusion.matrix_GSE21422.csv")
...
write.csv(nn.valid.matrics, "nn.valid.matrics_GSE21422.csv")
...
write.csv(nn.test.cm$table, paste0("nn.test.confusion.matrix_GSE21422.", accession, ".csv"))
...
write.csv(nn.test.matrics, paste0("nn.test.matrics_GSE21422.", accession, ".csv"))
######################################################################
